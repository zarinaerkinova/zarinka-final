...
const Card = ({ product }) => {
	...
	const toggleFavorite = async e => {
		e.stopPropagation()
		if (!user || !token)
			return toast.error('Please log in to manage favorites.')
	...
	const rawPath = product?.image || '/uploads/default.png'
	const safePath = rawPath.startsWith('http')
		? rawPath
		: `${backendBase}${rawPath}`
	const imageUrl = encodeURI(safePath)
				...
				{user && (
					<button
						className={`${styles['favorite-btn']} ${
							favorites?.some(p => p._id === product._id) ? styles.active : ''
						}`}
